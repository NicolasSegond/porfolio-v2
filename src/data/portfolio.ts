export const personalInfo = {
  name: "Nicolas Segond",
  title: "Développeur Fullstack",
  bio: "Je build des apps de A à Z — du design system au pipeline Kubernetes. 3 ans d'alternance en conditions réelles, un mastère en poche, et une obsession pour le code propre et les archis scalables.",
  email: "nicolassegond0@gmail.com",
  linkedin: "https://www.linkedin.com/in/nicolassegond/",
  github: "https://github.com/NicolasSegond",
  photo: "/photo1.webp",
};

export const skills = {
  "Languages": ["TypeScript", "JavaScript", "PHP", "Python", "SQL", "HTML", "CSS"],
  "Frontend": ["React", "Next.js", "Tailwind CSS", "jQuery"],
  "Backend": ["Symfony", "API Platform", "Node.js"],
  "Data": ["MySQL", "PostgreSQL", "MongoDB", "Kafka", "RabbitMQ"],
  "DevOps": ["Docker", "Kubernetes", "CI/CD", "WSL / Linux"],
  "Infra & Auth": ["Traefik", "Kong", "Keycloak", "Ory Hydra"],
  "Monitoring": ["Prometheus", "Grafana"],
  "Outils": ["Git (Hub & Lab)", "WordPress", "Figma"],
};

export const timeline = [
  {
    year: "2018 — 2021",
    title: "Baccalauréat Général",
    location: "Lycée Albert Chatelet",
    description: "Spécialités NSI, Mathématiques et Physique-Chimie. Obtenu avec mention Bien.",
    type: "formation" as const,
  },
  {
    year: "2021 — 2023",
    title: "BTS SIO option SLAM",
    location: "Lycée André Malraux, Béthune",
    description: "Solutions Logicielles et Applications Métiers. Développement web, mobile et client lourd. Projets en PHP/Symfony, Java Android, C#/.NET.",
    type: "formation" as const,
  },
  {
    year: "2022 · 4 semaines",
    title: "Stagiaire Développeur Web — Allographique",
    location: "Divion",
    description: "Conception et intégration de 6 sites vitrines sous WordPress/Elementor. Travail à partir de maquettes et cahiers des charges clients.",
    type: "experience" as const,
  },
  {
    year: "2023 · 7 semaines",
    title: "Stagiaire Développeur Web — Diggit-All",
    location: "Lens",
    description: "Développement de sites WordPress/Divi, création de supports print (plaquettes, cartes de visite), relation client.",
    type: "experience" as const,
  },
  {
    year: "2023 — 2024",
    title: "Bachelor CDA (Concepteur Développeur d'Applications)",
    location: "CESI — en alternance chez Topo Marketing Group",
    description: "Formation Bac+3 en alternance. Approfondissement en conception logicielle, architecture d'applications et méthodologies agiles.",
    type: "formation" as const,
  },
  {
    year: "2023 — 2026",
    title: "Développeur Fullstack — Topo Marketing Group",
    location: "Villeneuve-d'Ascq",
    description: "Création de Dynamic Reports, maintenance et évolution d'applications en production, gestion de projets, recueil de besoins clients, mise en place d'architectures microservices, CI/CD, monitoring.",
    type: "experience" as const,
  },
  {
    year: "2024 — 2026",
    title: "Mastère MAALSI",
    location: "CESI — en alternance chez Topo Marketing Group",
    description: "Manager en Architecture et Applications Logicielles des Systèmes d'Information. Bac+5 en alternance. Architecture logicielle, management SI, sécurité, DevOps.",
    type: "formation" as const,
  },
];

export const projects = [
  {
    slug: "portfolio-v2",
    title: "Portfolio v2",
    subtitle: "Site personnel — design macOS",
    category: "personnel",
    year: "2025",
    description: "Portfolio interactif reproduisant l'environnement macOS : fenêtres draggables, dock, terminal, animations fluides. Construit avec Next.js, Tailwind CSS et Framer Motion.",
    tech: ["Next.js", "React", "TypeScript", "Tailwind CSS", "Framer Motion"],
    image: "/projects/portfolio-v2.webp",
    repo: "https://github.com/NicolasSegond/porfolio-v2",
    demo: "https://nicolassegond.fr",
    details: {
      context: "Projet personnel de refonte complète de mon portfolio. L'idée : reproduire un environnement macOS dans le navigateur pour présenter mon parcours, mes projets et mes compétences de manière originale.",
      features: [
        "Interface macOS complète : fenêtres draggables et redimensionnables, dock, barre de menu",
        "Animations fluides avec Framer Motion (transitions, ouverture/fermeture de fenêtres)",
        "Terminal interactif avec commandes personnalisées",
        "Navigation façon Safari avec vue liste et vue détail des projets",
        "Design responsive (adaptation mobile automatique)",
        "Déploiement Docker avec hot-reload",
      ],
      role: "Développeur fullstack — design, développement et déploiement du projet de A à Z.",
    },
  },
  {
    slug: "microservice-collection",
    title: "MicroService Collection",
    subtitle: "Plateforme marketplace microservices",
    category: "etude",
    year: "2025",
    description: "POC d'une plateforme marketplace moderne construite avec une architecture microservices event-driven. Communication asynchrone via Kafka, authentification OAuth2/OIDC avec Keycloak, déploiement Kubernetes.",
    tech: ["Symfony", "React", "Kafka", "Keycloak", "Kubernetes", "Docker"],
    image: "/projects/microservice-collection.webp",
    repo: "https://github.com/NicolasSegond/MicroService_Collection",
    demo: "",
    details: {
      context: "Projet réalisé dans le cadre du Mastère MAALSI au CESI. Conception d'une marketplace avec des services découplés communiquant de manière asynchrone via Apache Kafka, chacun disposant de sa propre base PostgreSQL.",
      features: [
        "Architecture event-driven avec Apache Kafka (aucun appel direct entre services)",
        "Authentification OAuth2/OIDC via Keycloak avec introspection JWT (Ory Oathkeeper)",
        "API Gateway Traefik avec routage et validation des tokens",
        "API REST sécurisée avec API Platform (rôles, ownership)",
        "Monitoring complet : Prometheus, Grafana, AlertManager",
        "CI/CD GitHub Actions : tests, SonarCloud, OWASP ZAP, JMeter",
        "Déploiement Kubernetes via Kustomize (Minikube)",
        "Synchronisation temps réel des utilisateurs Keycloak → Kafka → services",
      ],
      role: "Développeur fullstack & DevOps — conception de l'architecture microservices, développement du backend Symfony/API Platform, frontend React, mise en place de l'infra Kubernetes, CI/CD et monitoring.",
    },
  },
  {
    slug: "ressources-relationnelles",
    title: "Ressources Relationnelles",
    subtitle: "Plateforme de partage de ressources",
    category: "etude",
    year: "2024",
    description: "Plateforme fullstack de partage de ressources relationnelles avec API PHP, application web React et application mobile Flutter, le tout orchestré avec Docker.",
    tech: ["PHP", "React", "Flutter", "Docker", "MySQL"],
    image: "/projects/ressources-relationnelles.webp",
    repo: "https://github.com/NicolasSegond/CESI_Ressource_Relationnel",
    demo: "",
    details: {
      context: "Projet réalisé dans le cadre du Bachelor CDA au CESI. Développement d'une plateforme complète de partage de ressources relationnelles, avec une API backend, un front web et une app mobile.",
      features: [
        "Authentification sécurisée avec tokens et refresh tokens",
        "Ajout et consultation de ressources (web + mobile)",
        "Système de favoris et sauvegarde de ressources",
        "Commentaires et pièces jointes sur les ressources",
        "Dashboard administrateur avec statistiques",
        "Notifications email à l'inscription",
        "Application mobile Flutter avec synchronisation API",
        "Déploiement Docker (API + front sur réseau bridge)",
      ],
      role: "Développeur fullstack — développement de l'API PHP, de l'application web React et de l'application mobile Flutter.",
    },
  },
  {
    slug: "gsb-extranet",
    title: "GSB Extranet",
    subtitle: "Application web de gestion de frais",
    category: "etude",
    year: "2022",
    description: "Application complète pour la gestion des frais des visiteurs médicaux du laboratoire Galaxy Swiss Bourdin. Système d'authentification, CRUD des fiches de frais avec états, interface comptable.",
    tech: ["PHP", "Symfony", "MySQL", "Bootstrap", "MVC"],
    image: "/projects/photo-gsb.webp",
    repo: "",
    demo: "",
    details: {
      context: "Projet réalisé durant le BTS SIO SLAM. Simulation d'un cas réel de gestion de frais pour un laboratoire pharmaceutique.",
      features: [
        "Authentification sécurisée avec gestion de rôles (visiteur, comptable)",
        "CRUD complet des fiches de frais avec workflow d'états (créée, clôturée, validée, remboursée)",
        "Interface dédiée comptable pour le traitement et la validation des demandes",
        "Gestion des frais forfaitisés et hors-forfait",
      ],
      role: "Développeur fullstack — conception de la base de données, développement back-end en PHP/Symfony, intégration front-end Bootstrap.",
    },
  },
  {
    slug: "gsb-mobile",
    title: "GSB Mobile",
    subtitle: "Application Android",
    category: "etude",
    year: "2023",
    description: "Version mobile permettant aux visiteurs médicaux de saisir leurs frais en déplacement. Synchronisation API REST, mode hors-ligne, Material Design.",
    tech: ["Java", "Android Studio", "API REST", "SQLite"],
    image: "/projects/gsbmobile.png",
    repo: "",
    demo: "",
    details: {
      context: "Extension mobile du projet GSB, développée durant le BTS SIO SLAM pour permettre la saisie de frais en mobilité.",
      features: [
        "Saisie de frais en déplacement avec interface Material Design",
        "Synchronisation avec l'API REST du serveur",
        "Mode hors-ligne avec stockage SQLite local",
        "Notification de statut des fiches de frais",
      ],
      role: "Développeur mobile — développement de l'application Android en Java, intégration de l'API REST.",
    },
  },
  {
    slug: "gsb-lourd",
    title: "GSB Client Lourd",
    subtitle: "Application de bureau",
    category: "etude",
    year: "2022",
    description: "Application desktop pour la gestion des visiteurs médicaux, comptes-rendus de visite et suivi des échantillons.",
    tech: ["C#", ".NET", "Windows Forms", "SQL Server"],
    image: "/projects/GSBLourd.webp",
    repo: "",
    demo: "",
    details: {
      context: "Application desktop développée durant le BTS SIO SLAM pour la gestion interne du laboratoire GSB.",
      features: [
        "Gestion des visiteurs médicaux et de leurs secteurs",
        "Saisie et consultation des comptes-rendus de visite",
        "Suivi des échantillons distribués",
        "Interface Windows Forms connectée à SQL Server",
      ],
      role: "Développeur — conception et développement de l'application en C#/.NET.",
    },
  },
];

export const certifications = [
  {
    name: "HTML5",
    source: "LinkedIn Learning",
    image: "/certifications/html5.webp",
  },
  {
    name: "PHP et MySQL",
    source: "LinkedIn Learning",
    image: "/certifications/php-mysql.webp",
  },
  {
    name: "Fondements du graphisme",
    source: "LinkedIn Learning",
    image: "/certifications/graphisme.webp",
  },
  {
    name: "Pix",
    source: "Pix.fr",
    image: "/certifications/pix.webp",
  },
];

export const projectCategories = [
  { value: "all", label: "Tous" },
  { value: "etude", label: "Projets d'étude" },
  { value: "personnel", label: "Projets perso" },
];
